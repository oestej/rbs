...
def test_pre_v8_files_are_rejected(tmp_path) -> None:
...
def test_v7_files_are_upgraded_with_electives_available_all_year(tmp_path) -> None:
    source = _store(tmp_path / "a")
    payload = json.loads(source.export_workspace_rbsc(_workspace(source).id))
    payload["schema_version"] = 7
    for record in payload["catalogs"]:
        record["catalog"]["schema_version"] = 6
        for option in record["catalog"]["electives"]["rotation_options"]:
            option.pop("blackout_weeks")

    target = _store(tmp_path / "b")
    imported = target.import_workspace_rbsc(json.dumps(payload))[0]

    assert imported.instance.electives.rotation_options
    assert all(
        not option.blackout_weeks
        for option in imported.instance.electives.rotation_options
    )
