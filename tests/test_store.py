...
def test_rbsc_round_trip_replaces_and_restores_the_complete_database(tmp_path) -> None:
    ...
    assert exported["schema_version"] == 11
...
def test_rbsc_v11_omits_application_preferences_and_derived_locks(tmp_path) -> None:
    from rbs.models.instance import SchedulerInput
...
def test_rbsc_v11_coalesces_catalogs_that_differ_only_by_colors(tmp_path) -> None:
    from rbs.models.instance import SchedulerInput
...
def test_persisted_v7_catalogs_are_rejected_on_load(tmp_path) -> None:
    ...
    for group in legacy["rotation_groups"]:
    ...
    with store.connect() as connection:
        catalog_id = connection.execute(
            ...
            INSERT INTO catalogs
                (name, schema_version, content_hash, catalog_json, created_at, updated_at)
            VALUES ('Legacy v7', 7, 'legacy-v7-hash', ?, 'now', 'now')
            """,
            (json.dumps(legacy),),
        ).lastrowid
        workspace_id = connection.execute(
